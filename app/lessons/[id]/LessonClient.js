...
export default function LessonClient({ lesson }) {
  ...
  useEffect(() => {
    try {
      const saved = JSON.parse(sessionStorage.getItem('tarok_learned_' + lesson.id) || '[]')
  ...
  const markLearned = (idx) => {
    ...
    setLearned(next)
    try {
      sessionStorage.setItem('tarok_learned_' + lesson.id, JSON.stringify([...next]))
      const prog = JSON.parse(sessionStorage.getItem('tarok_progress') || '{}')
      prog[lesson.id] = next.size
